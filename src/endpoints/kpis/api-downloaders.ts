...
const getSandboxAPIDownloaders: ApiHandler<KPIFigureResponse> = async (request: Request) => {
  let response;
  ...
  try {
    const result = await DB.query(QUERIES.getAPIDownloaders(), [fromDate, toDate]);
...
router.get("/", respondWith(getSandboxAPIDownloaders));
