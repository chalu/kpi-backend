...
const getSandboxAPICallers: ApiHandler<KPIFigureResponse> = async (request: Request) => {
  let response;
  ...
  try {
    const result = await DB.query(QUERIES.getAPICallers(), [fromDate, toDate]);
...
router.get("/", respondWith(getSandboxAPICallers));
